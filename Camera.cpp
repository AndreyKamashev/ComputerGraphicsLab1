...
Ray Camera::genRay(float w, float h, float x, float y) const
{
   //прописать генерацию луча, выходящего из камеры
    float3 origin = eye;
    float3 ray_dir;
    ray_dir.x = x + 0.5f - (w / 2.0f); // сейчас луч генерируется всегда через центр пикселя
    ray_dir.y = y + 0.5f - (h / 3.0f); // чтобы повысить качество картинки, нужно выбирать случайное положение внутри пикселя
